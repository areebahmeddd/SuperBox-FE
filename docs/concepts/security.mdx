...
<Frame>
  <div className="bg-gradient-to-r from-red-500 via-orange-500 to-green-500 p-6 rounded-lg text-white">
    <h3 className="text-2xl font-bold mb-4">Zero-Trust Security Model</h3>
    <p>Every MCP server undergoes rigorous automated security scanning before deployment.</p>
  </div>
</Frame>
...
<Tabs>
  ...
  <Tab title="Execution">
    ...
    sonar-scanner \
      -Dsonar.host.url=https://sonarqube.superbox.com \
      -Dsonar.login=$SONAR_TOKEN \
      -Dsonar.projectKey=$PROJECT_KEY
    
    # Wait for quality gate
    quality_gate=$(curl -s -u $SONAR_TOKEN: \
      "https://sonarqube.superbox.com/api/qualitygates/project_status?projectKey=$PROJECT_KEY" \
...
<Tabs>
  ...
  <Tab title="Score Grades">
    ...
    | 75-84 | B | ⚠️ Fair - Manual review |
    | 65-74 | C | ⚠️ Needs improvement |
