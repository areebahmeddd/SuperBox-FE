...
<Frame>
  ```mermaid
  graph TD
      A[AI Client] --> B[Local Proxy - stdio]
      B --> C[WebSocket API Gateway]
      C --> D[Lambda Function]
      D --> E[Sandbox Environment]
      
      subgraph E[Lambda Sandbox]
          F[Fetch Metadata from S3]
          G[Clone GitHub Repository]
          H[Install Dependencies]
          I[Execute MCP Server]
          J[Stream Response]
...
<Tabs>
  <Tab title="Pricing Model">
    AWS Lambda pricing (INR):
    
    - **Requests:** ₹17 per 1M requests
    - **Duration:** ₹0.0014 per GB-second
    ...
    Request cost: (10M - 1M) × ₹17 / 1M = ₹153
    Duration cost: 10M × 0.25s × 1GB × ₹0.0014 = ₹3,500
