...
<Step title="Sandbox Bootstrap">
  ...
  to `/tmp` directory 3. Install Python/Node.js dependencies 4. Load MCP server
  configuration 5. Initialize server instance
</Step>

<Step title="Tool Execution">
  Execute requested tool in sandbox: ```python # Inside Lambda handler async def
  execute_tool(server_id, tool_name, parameters): # Load server server = await
  load_mcp_server(server_id) # Call tool result = await server.call_tool(
  name=tool_name, arguments=parameters ) return result ```
</Step>
...
Blocked: - All inbound connections - Internal AWS service access - Cross-VPC communication - Direct database access
...
<Accordion title="Environment Variables" icon="gear">
  Available to MCP servers: ```bash AWS_REGION=us-east-1
  AWS_EXECUTION_ENV=AWS_Lambda_python3.11 LAMBDA_TASK_ROOT=/var/task
  LAMBDA_RUNTIME_DIR=/var/runtime # Custom variables SERVER_ID=weather-mcp-123
  SUPERBOX_ENV=production LOG_LEVEL=info ```
</Accordion>
