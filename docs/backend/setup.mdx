...
<Accordion title="go test ./..." icon="vial">
  Run all tests with coverage ```bash go test ./... -v -cover # Generate
  coverage report go test ./... -coverprofile=coverage.out go tool cover
  -html=coverage.out ```
</Accordion>

<Accordion title="go build" icon="hammer">
  Build production binary ```bash # Build for current OS go build -o
...
<Accordion title="Database Commands" icon="database">
  Manage database migrations ```bash # Run migrations go run cmd/migrate/main.go
...
<Accordion title="Redis Connection Error" icon="bolt">
  Verify Redis is running: ```bash docker-compose ps redis ``` Test connection:
  ```bash redis-cli ping ```
</Accordion>

<Accordion title="Go Module Errors" icon="golang">
  Clean and re-download modules: ```bash go clean -modcache go mod download go
  mod tidy ```
</Accordion>

<Accordion title="Python Import Errors" icon="python">
  Reinstall dependencies: ```bash pip uninstall superbox -y pip install -e
  ".[dev]" ```
</Accordion>
