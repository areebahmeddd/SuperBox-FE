...
<Steps>
  ...
  <Step title="Configure Environment">
    ...
    # CORS
    CORS_ALLOWED_ORIGINS=http://localhost:3000,https://superbox.ai
    ```
...
<AccordionGroup>
  ...
  <Accordion title="go build" icon="hammer">
    ...
    go build -o bin/superbox-api cmd/server/main.go
    
    # Build for Linux
    GOOS=linux GOARCH=amd64 go build -o bin/superbox-api-linux cmd/server/main.go
    
    # Build with optimization
    go build -ldflags="-s -w" -o bin/superbox-api cmd/server/main.go
    ```
  </Accordion>
