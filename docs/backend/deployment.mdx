...
<Step title="AWS CLI">
  Install and configure AWS CLI: ```bash
# Install AWS CLI curl
...
``` Enter your AWS credentials: ``` AWS Access Key ID: your_access_key AWS
  Secret Access Key: your_secret_key Default region name: ap-south-1 Default
  output format: json ```
</Step>

<Step title="Terraform">
  Install Terraform for infrastructure as code: ```bash
# Download Terraform
  wget
...
<Step title="Create terraform.tfvars">
  ```hcl aws_region = "ap-south-1" environment = "production" app_name =
