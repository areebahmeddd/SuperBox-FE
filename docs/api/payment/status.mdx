...
<ResponseField name="status" type="string">
  Payment status: - `created` - Order created, awaiting payment - `authorized` -
  Payment authorized - `captured` - Payment captured - `failed` - Payment failed
...
<Warning>
  Use polling sparingly. Prefer webhook notifications for production
  applications.
</Warning>
