...
<Card>
  <code>GET https://api.superbox.dev/api/v1/servers/:name</code>
</Card>
...
func main() {
    ...
    url := fmt.Sprintf("https://api.superbox.dev/api/v1/servers/%s", serverName)
  ...
  "data": {
    ...
    "security_report": {
      "metadata": {
        ...
        "repo_url": "https://github.com/johndoe/weather-mcp",
...
<AccordionGroup>
  ...
  <Accordion title="Check Security Status" icon="shield">
    ...
    def is_server_secure(server_name):
        response = requests.get(
            f'https://api.superbox.dev/api/v1/servers/{server_name}'
        )
        server = response.json()['data']
  ...
  <Accordion title="Clone Repository" icon="code-branch">
    ...
    REPO_URL=$(curl -s "https://api.superbox.dev/api/v1/servers/weather-mcp" | \
