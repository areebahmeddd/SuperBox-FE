...
<Card>
  <code>PUT https://api.superbox.dev/api/v1/servers/:name</code>
</Card>
...
curl -X PUT "https://api.superbox.dev/api/v1/servers/weather-mcp" \
...
const response = await fetch(
  'https://api.superbox.dev/api/v1/servers/weather-mcp',
  {
    method: 'PUT',
    headers: {
...
func updateServer(name string, updates ServerUpdate) error {
    url := fmt.Sprintf("https://api.superbox.dev/api/v1/servers/%s", name)
...
def get_update_history(server_name):
    url = f"https://api.superbox.dev/api/v1/servers/{server_name}/history"
