...
<ParamField path="name" type="string" required>
  Unique server identifier to update
  
  **Example:** `weather-mcp`, `database-query-mcp`
</ParamField>
...
<Tabs>
  ...
  <Tab title="Description">
    ...
    **Good:**
    ```
    "Get real-time weather data, 10-day forecasts, 
  ...
  <Tab title="Repository">
    ...
    **Side effects:**
    - Server temporarily unavailable during scan
    - Deployment may be rejected if scan fails
    - Takes 1-2 minutes
  </Tab>
</Tabs>
...
<AccordionGroup>
  <Accordion title="Version Conflict" icon="triangle-exclamation">
    ...
    **Solution:** Check current version first, then increment appropriately
    
    ```javascript
    const current = await getCurrentVersion('weather-mcp');
  ...
  <Accordion title="Unauthorized" icon="lock">
    ...
    **Solution:** Ensure you're the server owner and using the correct token
  </Accordion>
  
  <Accordion title="Invalid Repository" icon="github">
    **Error:** Unable to access new repository URL
    
    **Solution:** Verify the repository is public and URL is correct
  </Accordion>
</AccordionGroup>
