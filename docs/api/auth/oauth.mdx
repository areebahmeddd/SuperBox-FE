...
api: "GET https://api.superbox.ai/v1/auth/oauth/{provider}"
icon: "link"
---
...
<Step title="Callback">
  ...
  https://superbox.ai/auth/callback?code=abc123&state=xyz ```
</Step>
...
```javascript
JavaScript
window.location.href = "https://api.superbox.ai/v1/auth/oauth/google";
```
...
response = requests.get('https://api.superbox.ai/v1/auth/oauth/google')
...
export default async function handler(req, res) {
  ...
  const response = await fetch(
    "https://api.superbox.ai/v1/auth/oauth/callback",
    {
      method: "POST",
