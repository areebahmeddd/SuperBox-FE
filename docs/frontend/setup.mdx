...
<Accordion title="npm run build" icon="hammer">
  Creates an optimized production build ```bash npm run build ``` Output is
  generated in the `.next` directory
</Accordion>

<Accordion title="npm run start" icon="rocket">
  Starts the production server (requires build first) ```bash npm run build &&
  npm run start ``` Serves the optimized production build
</Accordion>

<Accordion title="npm run lint" icon="circle-check">
  Runs ESLint to check code quality ```bash npm run lint ``` Identifies and
...
<Accordion title="Module not found errors" icon="circle-exclamation">
  Clear node_modules and reinstall: ```bash rm -rf node_modules
  package-lock.json npm install ```
</Accordion>

<Accordion title="TypeScript errors" icon="code">
  Restart the TypeScript server in VS Code: 1. Press `Cmd/Ctrl + Shift + P` 2.
  Type "TypeScript: Restart TS Server" 3. Press Enter
</Accordion>

<Accordion title="Tailwind styles not working" icon="paintbrush">
  Ensure Tailwind is properly configured and rebuild: ```bash npm run build npm
  run dev ```
</Accordion>
