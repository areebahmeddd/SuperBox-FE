...
<Tabs>
  <Tab title="Module Import Error">
    ...
    **Fix:**
    ...
    echo "xyz==1.2.3" >> SuperBox-BE/requirements.txt
...
<AccordionGroup>
  ...
  <Accordion title="Connection Refused" icon="plug">
    **Error:** ``` Error: Could not connect to SuperBox backend ``` **Causes:**
    - Backend server down - Network/firewall blocking connection - Incorrect API
    endpoint **Debug:** ```powershell # Test connectivity curl
    https://api.superbox.dev/health # Check DNS resolution nslookup
    api.superbox.dev # Verify endpoint in config superbox config get api_url ```
  </Accordion>
</AccordionGroup>
...
<Steps>
  <Step title="Access Lambda Logs">
    ```powershell aws logs tail /aws/lambda/superbox-mcp-executor --follow ```
  </Step>
  ...
  <Step title="Extract Stack Traces">
    ```sql fields @timestamp, @message | filter @message like /Traceback/ |
    display @message ```
  </Step>
</Steps>
