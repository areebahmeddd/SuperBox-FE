...
<AccordionGroup>
  <Accordion title="AWS Account" icon="aws">
    - Active AWS account with billing enabled - Programmatic access (Access Key
    ID + Secret Access Key) - Permissions to create: S3, Lambda, IAM, CloudWatch
    resources - Recommended: Admin access or custom IAM policy
  </Accordion>

  <Accordion title="OpenTofu/Terraform" icon="cube">
    **OpenTofu (Recommended):** ```bash # Windows winget install OpenTofu.tofu #
    macOS brew install opentofu # Linux curl -fsSL
    https://get.opentofu.org/install-opentofu.sh | bash ``` **Terraform
    (Alternative):** ```bash # Download from https://www.terraform.io/downloads
    # Extract and add to PATH terraform version ```
  </Accordion>

  <Accordion title="AWS CLI (Optional)" icon="terminal">
    For verification and testing: ```bash # Install AWS CLI pip install awscli #
...
<Steps>
  ...
  <Step title="Create Access Key">
    - Go to **Security Credentials** tab - Click **Create Access Key** - Choose
    **CLI/SDK** as use case - Download or copy the credentials
  </Step>

  <Step title="Store Securely">
    ```bash # Never commit these to git! AWS_ACCESS_KEY_ID=AKIAIOSFODNN7EXAMPLE
...
<Tabs>
  <Tab title="PowerShell (Windows)">
    ```powershell # Navigate to scripts directory cd SuperBox-Infra/scripts #
    Run packaging script .\package_lambda.ps1 # Verify output ls
    ../modules/lambda/lambda_payload.zip ```
  </Tab>

  <Tab title="Bash (Linux/macOS)">
    ```bash # Navigate to scripts directory cd SuperBox-Infra/scripts # Make
    script executable chmod +x package_lambda.sh # Run packaging script
    ./package_lambda.sh # Verify output ls -lh
    ../modules/lambda/lambda_payload.zip ```
  </Tab>
</Tabs>
...
<Steps>
  <Step title="Navigate to Infrastructure Directory">
    ```bash cd SuperBox-Infra ```
  </Step>

  <Step title="Initialize OpenTofu/Terraform">
    ```bash # OpenTofu tofu init # OR Terraform terraform init ``` This
    downloads required providers and initializes backend. **Expected Output:**
    ``` Initializing modules... Initializing the backend... Initializing
    provider plugins... - Finding latest version of hashicorp/aws... -
    Installing hashicorp/aws v5.x.x... OpenTofu has been successfully
    initialized! ```
  </Step>

  <Step title="Validate Configuration">
    ```bash # OpenTofu tofu validate # OR Terraform terraform validate ```
...
<Steps>
  ...
  <Step title="Plan Changes">
    ```bash tofu plan -out=tfplan ``` Review what will change
  </Step>

  <Step title="Apply Updates">```bash tofu apply tfplan ```</Step>
</Steps>
