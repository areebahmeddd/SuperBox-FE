...
<Tip>
  Review the plan carefully. Verify bucket names, IAM permissions, WebSocket API Gateway, and Lambda
...
<Tabs>
  <Tab title="Test WebSocket Connection">
    ```bash
...
```
  ...
  <Accordion title="Error: Region not supported" icon="triangle-exclamation">
    ...
    **Solution:**
    ...
    - Use valid region code (e.g., `ap-south-1`)
